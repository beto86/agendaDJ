...
def submit_evento(request):
	if request.POST:
		titulo = request.POST.get('titulo')                   #faz a requisição
		data_evento = request.POST.get('data_evento')
		descricao = request.POST.get('descricao')
		usuario = request.user
		id_evento = request.POST.get('id_evento')
		if id_evento:
			Evento.objects.filter(id=id_evento).update(titulo=titulo,           # edita
 													   data_evento=data_evento,
 													   descricao=descricao)
		else:
			Evento.objects.create(titulo=titulo,      # cria (registra)
								  data_evento=data_evento,
				  				  descricao=descricao,
					  			  usuario=usuario)
